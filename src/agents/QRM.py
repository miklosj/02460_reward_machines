from agents.utils import maxAction_QRM as maxAction
import numpy as np
...
def QRM_agent(env):
    ...
    for i in range(numGames):
        ...
        while not done:
            ...
            # QRM learning loop
            for Q_rmstate in range(env.rmstate_space):
                Q_rmstate_ = env.give_rmstate(Q_rmstate, state_)
                Q_reward = env.give_reward(Q_rmstate, Q_rmstate_)
                if Q_rmstate_ == 2:
                    Q_rmstate_ = 1
                Q[state, action, Q_rmstate] = Q[state, action, Q_rmstate] + alpha*(Q_reward+ \
                                            gamma *Q[state_, action_, Q_rmstate_] - Q[state, action, Q_rmstate])
