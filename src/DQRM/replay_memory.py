...
class ReplayBuffer(object):
    def __init__(self, max_size, input_shape, n_actions, n_rm_states):
        ...
        self.u1_memory = np.zeros((self.mem_size, n_rm_states), dtype=np.int64)
        self.u2_memory = np.zeros((self.mem_size, n_rm_states), dtype=np.int64)
        self.reward_memory = np.zeros((self.mem_size, n_rm_states), dtype=np.float32)
