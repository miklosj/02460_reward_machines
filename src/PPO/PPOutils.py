...
class Storage():
	def __init__(self, obs_shape, mem_size, gamma=0.99, lmbda=0.95, normalize_advantage=True):
	...
	def reset(self):
		self.obs = torch.zeros(self.mem_size+1, *self.obs_shape)
		self.action = torch.zeros(self.mem_size)
		self.reward = torch.zeros(self.mem_size)
		self.done = torch.zeros(self.mem_size)
		self.log_prob = torch.zeros(self.mem_size)
		self.value = torch.zeros(self.mem_size+1)
		self.returns = torch.zeros(self.mem_size)
		self.advantage = torch.zeros(self.mem_size)
		self.info = deque(maxlen=self.mem_size)
		self.step = 0
		self.test_info = deque(maxlen=self.mem_size)
	...
	def get_generator(self, batch_size=1024):
		iterator = BatchSampler(SubsetRandomSampler(range(self.mem_size)), batch_size, drop_last=True)
	...
	def get_reward(self, normalized_reward=True):
		if normalized_reward:
			reward = []
			for step in range(self.mem_size):
				info = self.info[step]
